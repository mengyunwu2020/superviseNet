...
generate.data = function(n,Mu0.list,beta,beta0,sigma){
  ...
  N <- rep(n,K0)
  p = length(Mu0.list[[1]])
  A.list <-Power.law.network3(p,s=10,umin=0.5,umax=1,num.differ=num.differ,Sparse=TRUE,m=1,Dnum=3,rate=1,option='ne')
  rate=.8
  Theta01 <- A.list$A1
  Theta02 <- A.list$A2
  Theta03 <- A.list$A3
  ...
  for(kk in 1:K0){
    ...
    while((abs(sum(delta)/N[kk]-rate)>0.02)&niter<=50){
      niter=niter+1
      censoring.time<-rgamma(round(N[kk]*0.5), shape =10, scale =10)  # censoring time
      cc<-sample(c(censoring.time,rep(exp(9999),N[kk]-round(N[kk]*0.5))),N[kk])
      time<- pmin(ti[(ntmp):(ntmp+N[kk]-1)], cc)  # observed time is min of censored and true
      delta = time == ti[(ntmp):(ntmp+N[kk]-1)] # set to 1 if event is observed
    ...
    if(niter==51){
      ...
      for(i in ss){
        censoring.time<-rgamma(round(N[kk]*0.5), shape =i, scale =i)  # censoring time
        cc<-sample(c(censoring.time,rep(exp(9999),N[kk]-round(N[kk]*0.5))),N[kk])
        time<- pmin(ti[(ntmp):(ntmp+N[kk]-1)], cc)  # observed time is min of censored and true
